...
class EllipticalSliceSampler(SamplingLoop):
    ...
    def run(self):
        ...
        while not self.is_converged():
            ...
            # print(f'GUY ({self.loop_state.iteration})')
    ...
    def compute_next_point(self, x0):
        ...
        while tries:
            ...
            if not active_intersections.ellipse_in_domain:
                tries -= 1
                # print('bad ellipse is x0=(%.3f, %.3f), x1=(%.3f, %.3f)' %(x0[0],x0[1],x1[0],x1[1]))
                # breakpoint()
                # ellipse is outside of integration domain, reconstruct a new ellipse (should not happen at all!)
                if(tries):
                    # print('At least one point should be in the domain! trying again')
                    pass
                else:
